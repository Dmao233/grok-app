...
describe("resolveWorkbenchPaneOverlay", () => {
  ...
  it("does not grow the window when opening the sidebar leaves the aside overlaid", () => {
    const start = appWorkbench.indexOf("const openSidebarPane = useCallback");
    const end = appWorkbench.indexOf("const closeSidebarPane", start);
    const openSidebarPane = appWorkbench.slice(start, end);
