...
describe("desktop hidden CSS must not force width 0", () => {
  ...
  it("keeps the bottom terminal separator and surfaces theme-native", () => {
    ...
    expect(terminal).not.toMatch(/(?:background|border(?:-color)?):\s*(?:#000(?:000)?\b|black\b)/i);
  });
  ...
  it("interpolates the right-aligned side workbench between rail and full width", () => {
    ...
    expect(app).toContain("sideExpanded: sideWorkbench.expanded");
    ...
    expect(side).toMatch(
      /\.workbench--side-expand-motion\s+\.aside:not\(\.is-resizing\)\s*\{[^}]*width var\(--motion-pane\)[^}]*min-width var\(--motion-pane\)[^}]*max-width var\(--motion-pane\)/s,
    );
    expect(side).toMatch(
